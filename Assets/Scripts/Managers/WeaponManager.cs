...
public class WeaponManager : Singleton<WeaponManager>
{
	...
	private void Start()
	{
		...
		{
			allGuns.Find(x => x.GunSO.id == ownGun.id).Reload();
		});
	}
	...
	/// <summary>
	/// Returns ammunition for a given gun
	/// </summary>
	/// <param name="gun"></param>
	/// <returns>The number of bullets to be loaded into the gun</returns>
	public int GetAmmoForReload(Gun gun)
	{
		if (gun == null) return 0;
		ShopItem availableBullets = GameManager.Instance.PermanentInventory.BoughtItems.Find(x => x.id == gun.GunSO.bulletShopItem.id);

		// Item missing
		if (availableBullets == null)
		{
			Debug.LogError($"No applicable ammo item found for gun {gun.GunSO.id}");
			return 0;
		}

		int bulletsToReload = Mathf.Min(gun.GunSO.magSize - gun.LoadedAmmunition, availableBullets.quantity);
		availableBullets.quantity -= bulletsToReload;
